...
<form method="post" rel="pagerForm" action="<%=baseUrl %>/assure/thirdparty/thirdsearch"  onsubmit="return dwzSearch(this, 'dialog');">
	...
	<div class="panelBar">
	 ...
	 <table class="table" width="100%"  layoutH="140">
		<thead align="center">
			<tr>
			    <th width="10%"></th>
				<th width="40%">全称(中文)</th>
				<th width="40%">全称(英文)</th>
				<th width="10%" >查找带回</th>
			</tr>
		</thead>
		<tbody align="center">
		<c:forEach var="item" items="${tdlist}" varStatus="s">
			<tr target="slt_objId" rel="${item.oid}">
			    <td>${s.index + 1}</td>
				<td>${item.namecn}</td>
				<td>${item.nameen}</td>
				<td>
				<div>
					<a class="btnSelect" title="查找带回" href="javascript:$.bringBack({oid:'${item.oid}',namecn:'${item.namecn}',nameen:'${item.nameen}'})">选择
 				 	</a>
 				 </div>
				</td>
			</tr>

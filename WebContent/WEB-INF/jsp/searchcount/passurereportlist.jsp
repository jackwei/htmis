...
<c:import url="../layout/pager/pagerForm.jsp"></c:import>
    ...
    <div class="panel" >
		<form method="post" rel="pagerForm" action="<%=baseUrl%>/searchcount/standardcondition?standardflag=passurereport" onsubmit="return navTabSearch(this)">
...
<div class="pageContent">
	<table class="table" width="100%" layoutH="53">
		...
		<tbody>
		<c:forEach var="item" items="${preportList}" varStatus="s">
			<tr >
				<td>${s.index + 1}</td>
				<td>${item.rewarranttype}</td>
				<td>${item.assurenum}</td>
				<td>${item.shipname}</td>
				<td>${item.assureder}</td>
				<td>${item.recognizorName}</td>
				<td>${item.casedes}</td>

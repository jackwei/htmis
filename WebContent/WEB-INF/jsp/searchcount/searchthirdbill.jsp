...
<c:import url="../layout/pager/pagerForm.jsp"></c:import>
	...
	<div class="panel" >
		<h1>第三方统计<a>${startdate} - ${enddate}</a></h1>
		<form method="post" rel="pagerForm" action="<%=baseUrl%>/searchcount/searchcondition?searchflag=thirdbill" onsubmit="return navTabSearch(this)">
...
<div class="pageContent">		
	<table class="table" width="100%" layoutH="80">
		...
		<tbody>
		<c:forEach var="item" items="${thirdList}" varStatus="s">
			<tr>
				<td>${s.index + 1}</td>
				...
				<td>${item.count}</td>
				<td><a class="button" target="navTab" rel="billcountNav" 
				href="<%=baseUrl %>/searchcount/searchcondition?searchflag=thirdinfos
				...
				&propertyname=${propertyname}&currency=${item.currency}&country=${country}&receiver=${receiver}&sender=${sender}&trackingNumber=${trackingNumber}&radio=${radio}&status=${item.status} " 

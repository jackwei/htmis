...
<div class="panel" >
        <h1>案件统计详细信息</h1>
		<form method="post" rel="pagerForm" action="<%=baseUrl%>/searchcount/searchcondition?searchflag=casecountdetail" onsubmit="return navTabSearch(this)">
		...
		<a id="casedetail" rel="caseNav" class="edit" target="navTab" href="<%=baseUrl %>/management/case/edit/{caseId}" title="案件信息"></a>
		</form>
	</div>
<div class="pageContent">
	<table class="table" width="100%" layoutH="80">
		...
		<c:forEach var="item" items="${caseList}" varStatus="s">
			<tr target="caseId" rel="${item.caseId }" ondblclick="casect()">
				<td>${s.index + 1}</td>

...
<form method="post" rel="pagerForm" action="<%=baseUrl %>/client/rewarranter/property"  onsubmit="return dwzSearch(this, 'dialog');">
	...
	<div class="searchBar">
		...
		<div class="subBar">
			<ul>						
				...
				<li><div class="buttonActive"><div class="buttonContent"><button onclick="$.bringBack({DictId:'',DictName:''});">清空</button></div></div></li>
			</ul>
		</div>
	</div>
</div>
...
<div class="panelBar">
	 ...
	 <table class="table" width="100%"  layoutH="200">
		<thead align="center">
			<tr>
				<th width="10%"></th>
				<th width="80%">反担保人评级</th>
				<!-- <th width="15%">查找带回</th> -->
			</tr>
		</thead>
		<tbody align="center">
		<c:forEach var="item" items="${menuList}" varStatus="s">
			<tr onclick="$.bringBack({DictId:'${item.DictId}', DictName:'${item.DictName}'});">
				<td>${item.SortNo}</td>
				<td>${item.DictName}</td>
<%-- 				<td>

...
//		if ("/".equals(requestUri)) requestUri = "/index!index.do";
		...
		for (String unsecureUri : this.unsecureUris) {
			...
			else if(unsecureUri.endsWith("*"))
			{
				uri = unsecureUri.substring(0, unsecureUri.indexOf("*")-1);
				int num = requestUri.indexOf(uri);
				if(num > 0)
				{
					unsecure = true;
				}
				...
				unsecure = requestUri.endsWith(uri);
			}
	...
	public void init(FilterConfig config) throws ServletException {
		...
		String uri = config.getInitParameter("excludedPages");
		if (uri != null) {
